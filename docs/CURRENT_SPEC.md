...
期限内に終わらなかったタスクを、完了でも削除でもない形で記録する。

- 付与は詳細モーダルの「❌ 失敗」ボタン（完了と削除の間）。`status:"failed"` と
  `failedAt`（ISO日時）を書く
- `active` の絞り込みから外れるので、バブル一覧からは消える
- 一覧は「📋 タスクサマリー」モーダル内のタブで完了済みと切り替える。
  別モーダルにしなかったのは、今週・今月の集計を見る場所が二手に分かれるため
- 失敗の行には失敗日に加えて元の期限も出す。「いつまでのものを落としたか」が
  分からないと振り返りにならない
- タブを切り替えるときはカテゴリの絞り込みを「すべて」に戻す。
  片方にしか無いカテゴリの絞り込みが残ると 0件に見える
- 「戻す」は完了と共通の `restore`（`status:"active"` に戻す）。
  `failedAt` は残すので、今週・今月の失敗件数はイベントとして数え続ける
...
アプリが `users/yokota/recEvents/<recId>__<kind>` に書く。

| kind | 何をしたとき |
|---|---|
| `open` | タイトル / 「🔗 詳細を見る」 / 「📍 場所」を開いた |
| `add` | ＋ でタスクにした |
| `dismiss` | − で却下した／👎 を押した |

**👎 はその場で却下まで済ませる。** 「いらない」と言ったものが一覧に残ると
結局あとで − を押し直すことになり、同じ判断を二度させられる。評価と却下は
同じ意思なので一度で終わらせる。ただし取り消し（同じ側をもう一度押す）では
消さない。誤タップを戻しただけなので。

キーを `recId__kind` にして**追記だけ**で済ませている。読んで直して
書き戻す形にすると、連打や複数端末で取りこぼすため。同じ反応の2回目は
同じキーを上書きするだけで害がない。
...
| 材料 | 解釈 | 扱い |
|---|---|---|
| タスク化した（note が「おすすめから追加」） | 当たり | 正例 |
| 👍 / 👎（recComments） | 明示的な意思表示 | 各2倍で加算 |
| 詳細を開いた（`open`） | 話題には関心がある | 弱い正例 |
| **出してから7日、何の反応も無く 👍👎 も無い** | 見る気にもならなかった | **弱い負例** |

最後のひとつが主役。タスク化と「−」だけでは利用者が手を動かしたものしか
見ておらず、実際に最も件数が多いのは「出しても何もされずに流れるもの」で、
興味の無い話題の情報はそこに最も多くある。

**「開いたが追加しなかった」は負例にしない。** 開いた時点で話題には興味が
あり、その回の中身が刺さらなかっただけ。ここを負例にすると関心のある分野
ごと消してしまう。

`add` / `dismiss` は recEvents では数えない。既に tasks の note と
dismissedTitles から拾っており、二重に数えると効きすぎる。

### 無反応の判定

`findIgnored()` が、現在の `recommendations` のうち
「recEvents に一件も反応が無い」かつ「**👍👎 も付いていない**」かつ
「`createdAt` から7日以上経過」を拾う。

**👍👎 は recEvents ではなく recComments に入る。** ここを見落とすと
「👍 を押したが詳細は開かなかった」ものが無反応と判定され、本人が明示的に
「もっと見たい」と言ったものを同時に負例として学習してしまう
（実際にこの取りこぼしがあった）。recComments に id は無いのでタイトルで
突き合わせる。
`createdAt` が無い古いおすすめは経過が測れないので対象外
（誤って負例にしないため）。日数は `IGNORED_AFTER_DAYS` で変えられる。

重みは毎回ゼロから計算し直すので、同じおすすめが何日も残っても
その回の集合に1件として入るだけで、累積して効きすぎることはない。
