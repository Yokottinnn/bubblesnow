...
`scripts/generate-recs.mjs` は `if (DRY_RUN)` の判定より**前**に Anthropic API を呼ぶ。
`DRY_RUN=true` がスキップするのは Firebase への書き込みだけで、**API 課金は発生する**。
「dry run だから無料」という思い込みが最も危険なので、ここを最初に疑うこと。

| 項目 | 値 |
| --- | --- |
| モデル | `claude-sonnet-4-20250514`（$3 / $15 per MTok） |
| `max_tokens` | 4000 |
| web検索 | `web_search_20250305` を最大 8 回（$10 / 1,000 検索） |
| 1回あたりの目安 | **$0.4〜$1.0** |
| 日次実行した場合 | **月 $12〜$30** |
