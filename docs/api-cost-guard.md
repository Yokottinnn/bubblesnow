...
**失敗しても課金は戻らない。** 2026-08-13 の dry-run は出力が `max_tokens` に達して
JSON が途中で切れ、$0.39 払って成果ゼロで終わった。そのため
`max_tokens` には余裕を持たせ、切れた配列からは完成した要素だけを救出するようにしてある
（救出処理は `MODE=validate` が見本で毎回検証する）。
