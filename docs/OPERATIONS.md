...
**期限切れか、3日以内に期限が来るものが1件でもあるときだけ**先頭に
メンションを付ける（`MENTION_KEYS`）。7日以内だけのときは付けない。
...
`status` が `active`（または未設定）以外のタスクは対象外。完了・失敗の
どちらも鳴らさない。

**判定は「done を外す」ではなく「active 以外を外す」向きで書くこと。**
「失敗」を足したとき done しか見ていなかったため、アプリで失敗にした
タスクが翌朝も期限切れとして鳴った（2026-09-21 に発生。9/20 13:01 に
失敗にしたものが 9/21 08:00 の通知に載った）。状態が増えるたびに同じ
取りこぼしをしないための書き方。

削除したタスクは配列から消えるので自動的に対象外になる。
