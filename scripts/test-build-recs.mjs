...
/* ── 材料が持つ期限とタイトルを尊重する ──
   メールの LLM 判定は、本文を読んで期限を取り出し、タイトルを
   行動が分かる形にして渡してくる。選別側でやり直すと両方が壊れる。

   ★実際に起きた壊れ方（2026-09-23）★
   ・期限 … 整形済みタイトルに「9月29日まで」の文字列が無いため
            本文からの正規表現が空振りし、支払期日が期限なしで出た
   ・語尾 … 「〜を支払う」にさらに動詞を足して
            「〜を支払うをチェックする」になった
   語尾の一覧を増やして追いかけると、キーワード照合と同じで
   別の動詞が出るたびに破れる。材料側の印を見ること。 */
{
  const r = toRec({
    title: 'ディズニー・クルーズライン残金を支払う',
    desc: '残金のお支払い期日が近づいています',
    deadline: '2026-09-29',
    titleIsAction: true,
    category: 'お金',
    via: 'gmail',
