   ...
   引っかかるので、精度はここで決まる。 */
{
  ...
  /* ★支払期日を丸ごと落としていた（2026-09-23 に発覚）★
     当時の語は「手続き」「期限」「お願い」などで、支払いを直接指す語が
     一つも無かった。同じ差出人の別件は末尾の「について」でたまたま
     通っており、拾えるかどうかが偶然に左右されていた。
     金銭と期限は落としてはいけない種類なので、実際に届いた件名で固定する。 */
  ok('実際に落とした支払期日を拾う',
    keepAction('【9月29日まで】ディズニー・クルーズラインの残金お支払い期日'));
  ok('同じ差出人の別件も拾う', keepAction('クルーズ残金お支払い期日について'));
  ok('請求を拾う', keepAction('【ご請求】10月分のご利用料金について'));
  ok('振込依頼を拾う', keepAction('お振込のお願い（9/30まで）'));
  ok('引き落としの事前通知を拾う', keepAction('口座振替のご案内 10月25日引き落とし'));
  ok('年会費を拾う', keepAction('年会費のお知らせ'));
  ok('未納を拾う', keepAction('料金未納のご連絡'));
  ok('自動更新の事前通知を拾う', keepAction('サブスクリプションの自動更新について'));
  ok('キャンセル期限を拾う', keepAction('無料キャンセル期限のご案内'));

  /* 支払いの「依頼」と「完了」を取り違えない。
     金銭の語を厚くしたぶん、済んだことの通知は確実に落とす。 */
  ok('支払い完了は拾わない', !keepAction('ご入金を確認いたしました'));
  ok('決済済みは拾わない', !keepAction('クレジットカード決済完了のお知らせ'));
  ok('返金完了は拾わない', !keepAction('返金処理が完了しました'));
